...
def create_val_mats(data, ques_idx, ans_idx, cap_idx, params):
    N = len(data['data']['dialogs'])
    num_round = 10
    max_ques_len = params.max_ques_len
    max_cap_len = params.max_cap_len
    ...
    image_list = []
    true_rounds = np.zeros(N, dtype='uint32')
    for i, img in enumerate(data['data']['dialogs']):
        image_id = img['image_id']
        image_list.append(image_id)
        #pdb.set_trace()
        ...
        true_rounds[i] = len(img['dialog'])
        while len(img['dialog']) <10:
            img['dialog'].append({"question": 1, "answer": 1})

        for ii in range(len(img['dialog'])):
            if 'answer' not in img['dialog'][ii]:
                img['dialog'][ii]['answer'] = 1
            if 'answer_options' not in img['dialog'][ii]:
                img['dialog'][ii]['answer_options'] = [1] * 100
    ...
    return captions, caption_len, questions, question_len, answers, answer_len, \
            options, options_list, options_len, image_list, true_rounds
...
if __name__ == "__main__":
    ...
    parser.add_argument('-input_json_val', default='/data/wanghui/dataset/VisDial_1.0/visdial_1.0_test.json', help='Input `test` json file')

    # Output files
    parser.add_argument('-output_json', default='/data/wanghui/dataset/VisDial_1.0/visdial_test_params.json', help='Output json file')
    parser.add_argument('-output_h5', default='/data/wanghui/dataset/VisDial_1.0/visdial_test_data.h5', help='Output hdf5 file')
    ...
    cap_val, cap_len_val, ques_val, ques_len_val, ans_val, ans_len_val, \
            opt_val, opt_list_val, opt_len_val, image_list_val , true_rounds= \
                create_val_mats(data_val, ques_idx_val, ans_idx_val, cap_idx_val, args)
    #pdb.set_trace()
    ...
    f.create_dataset('rounds', dtype='uint32', data=true_rounds)
    f.close()
